...
flowchart TD
    subgraph Build ["Build (build.ts)"]
        ...
        A2 --> A3["dist/@cody/codyx-{os}-{arch}[-baseline][-musl]/<br/>bin/codyx + package.json"]
    end

    subgraph Publish ["Publish (publish.ts)"]
        B1["bun run script/publish.ts"] --> B2["npm publish each platform package<br/>@cody/codyx-{os}-{arch}..."]
        B2 --> B3["Create wrapper @cody/codyx-ai<br/>bin: { codyx, cody }<br/>optionalDependencies: platform pkgs<br/>postinstall: postinstall.mjs"]
        B3 --> B4["npm publish @cody/codyx-ai"]
        B3 --> B5["Docker build & push<br/>ghcr.io/..."]
        B3 --> B6["Update AUR & Homebrew<br/>formulas"]
    end

    subgraph Install ["Install (end-user)"]
        C1["npm install @cody/codyx-ai"] --> C2["npm installs matching<br/>@cody/codyx-{os}-{arch} as<br/>optional dependency"]
        C2 --> C3["postinstall.mjs runs"]
        C3 --> C4["require.resolve('@cody/codyx-{p}-{a}')"]
        C4 --> C5["link binary → bin/.cody"]
    end
    ...
    subgraph Runtime ["Runtime"]
        ...
        D5 -->|yes| D4
        D5 -->|no| D6["walk node_modules for<br/>@cody/codyx-{p}-{a}/bin/codyx"]
        D6 --> D7{"found?"}
        D7 -->|yes| D4
