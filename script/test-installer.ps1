...
Assert-Contains "script/installer-verification.ps1" 'Your name \(required' "Verification helper requires the user's name"
Assert-Contains "script/installer-verification.ps1" 'Save-InstallerUsername' "Verification helper can persist the username to memo.md"
Assert-NotContains "script/installer-verification.ps1" 'if \(\$Yes\)' "Verification helper cannot be bypassed by -Yes"
Assert-Contains "script/install-npm.ps1" 'What would you like codyx to call you\?' "Npm installer requires a username before installation"
Assert-Contains "script/install-npm.ps1" '## User`r`n\$line' "Npm installer writes username under the memo.md User section"
Assert-Contains ".github/workflows/publish-npm.yml" 'build\.ts --all' "Npm publishing builds every platform package"
Assert-Contains "packages/codyx/script/publish.ts" 'Missing platform packages' "Npm publishing refuses incomplete platform packages"
Assert-Contains "packages/codyx/script/publish.ts" 'Platform package versions do not match' "Npm publishing refuses mixed platform versions"
Assert-Contains "packages/codyx/bin/_launcher.js" 'Codyx CLI \(Terminal UI\)' "Npm Windows launcher keeps the CLI choice"
Assert-Contains "packages/codyx/bin/_launcher.js" 'Codyx Web UI \(Browser\)' "Npm Windows launcher keeps the Web UI choice"
Assert-Contains "packages/codyx/bin/_launcher.js" '--print-banner-only' "Npm Windows launcher prints the banner before the menu"
Assert-Contains "packages/codyx/src/index.ts" 'Installation\.isLocal\(\) &&[\s\S]*tryAutoUpdateAsync' "Git startup updates only run for local source installs"
Assert-Contains "packages/codyx/src/session/prompt.ts" 'Hi \$\{username\}, it is nice to serve you' "First chat greets the saved username"
Assert-Contains "packages/codyx/src/session/prompt.ts" 'Yes please' "First chat offers the system-learning opt-in"
Assert-Contains "packages/codyx/src/session/prompt.ts" 'No, not now' "First chat offers the system-learning deferral"
Assert-Contains "packages/codyx/src/session/prompt.ts" 'permission: "\*".*action: "allow"' "Init opt-in grants temporary full permission"
